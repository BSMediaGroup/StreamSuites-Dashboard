...
test("economy styling includes compact identity rows and currency/denomination treatments", () => {
  ...
  assert.match(js, /economy-item-create-tags/);
  assert.match(js, /data-bulk-field="tags"/);
  assert.match(js, /Catalog tags \(comma-separated\)/);
  ...
  assert.match(css, /\.economy-item-tag-chip\s*\{[\s\S]*Blinker[\s\S]*font-weight:\s*400[\s\S]*rgba\(186,\s*220,\s*255/);
