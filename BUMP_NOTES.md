...
- Tester capacity summary: the Studio page renders Runtime/Auth-provided ALPHA stage, active invited non-admin tester count, maximum tester count, and derived remaining slots; it explains that admin access is automatic and admins do not consume the 25 invited tester slots.
- Existing-account selection: Add ALPHA tester reuses the same Runtime/Auth `/admin/accounts` source as the Accounts workspace, submits the stable account ID, supports the backend-confirmed optional 240-character administrative note, prevents duplicate/ineligible/admin rows in the selector, and does not change role, tier, account type, creator status, or other permissions.
- Grant lifecycle and state handling: added confirmed revoke and re-enable semantics, capacity gating with backend conflict handling, repeated-submission guards, refresh-after-success, success toasts, active/revoked filtering, search, loading/empty/session-expired/unauthorized/unavailable states, and Runtime/Auth error-code mapping for missing, duplicate, ineligible, validation, and cap-reached outcomes.
- Human note: Admin operators can now manage who is invited into Studio's closed ALPHA from the Dashboard while StreamSuites Runtime/Auth remains the only access authority. Room/media controls are not implemented; Cloudflare Realtime-first and later self-hosted LiveKit + Egress remain planned Studio architecture, not Dashboard functionality.
...
- Real raster cover image asset pack: replaced the rejected generated SVG primary cover pack with locally stored WebP city/capital covers under `docs/assets/analytics/location-covers/`, backed by `docs/shared/data/location-cover-images.json` schema v2 attribution/license metadata. Wikimedia Commons and Openverse were searched; every required city and country-capital fallback now resolves to a local real raster image, and the only generated raster fallback is the default unknown-location recovery image.
- Popup/sidebar cover sizing fix: MapLibre location popups now use an explicit 420px responsive card width, a bounded 16:9 cover frame, centered object-fit image rendering, padded text below the cover, and a visible light close button. The fullscreen selected-location sidebar uses the same controlled cover-frame behavior instead of fixed-height clipping.
- Test coverage: added `tests/location-cover-images.test.mjs` and updated `tests/analytics-map-fullscreen-polish.test.mjs` for real raster manifest coverage, local file existence, no remote image paths, no SVG primary covers, attribution/license completeness, sourceQuality requirements, cross-repo coverage equivalence, helper resolution, popup max-width/close styling, and sidebar/popup cover frame CSS.
...
- Alerts DanielClancy rule support: the Admin Alerts editor now has an additive Project selector (`StreamSuites`, `DanielClancy`) that filters backend-provided event types by source namespace, exposes DanielClancy trigger options from Runtime/Auth metadata, and keeps saved rule payloads on the existing `event_type` / `scope` / `destinations` contract.
- DanielClancy surface controls: the existing Match filters surface selector now includes `DanielClancy.net` (`danielclancy_public`) and `DanielClancy Admin` (`danielclancy_admin`), and DanielClancy event metadata defaults the editor toward the relevant surface without changing StreamSuites rule defaults.
- Human note: Dashboard remains a Runtime/Auth consumer/control surface only. Existing StreamSuites alert rules, Windows client targets, and Pushover destination options are preserved; DanielClancy live delivery still depends on Runtime/Auth `POST /api/alerts/danielclancy` env/config wiring.

- Admin Bots live snapping/reset root cause: the background polling path (`startPollingAsync()` -> `refresh()` -> `/api/admin/bots/status` -> `render(normalized)`) and manual refresh already used the state-preserving renderer, but the live app-shell path could still destroy and remount the Bots module on same-view hydration/visibility refresh. `destroy()` cleared `rowUi` and `expandedCreators`, so open creator rows, Twitch platform details, debug drawers, recent-message expansion, scroll, and focus were lost before the next status payload rendered.
- Admin Bots polling/remount fix: the Bots state layer now keeps stable `creator_id + platform` UI state across same-view teardown/remount, while the next payload render still prunes only rows that disappear. Background polling remains enabled, manual refresh uses the same preserving flow, and `session_id` rotation continues to follow the stable creator/platform row instead of collapsing Twitch debug state.
- Human note: open a Twitch creator row, platform detail, debug drawer, and Recent messages expansion; background polls, manual refresh, app-shell same-view remount, and Twitch `session_id` rotation should update text without snapping closed or jumping scroll.
- Kick protection note: Dashboard changes are limited to `docs/js/bots.js` and focused Bots polling-state tests. No Kick runtime, OAuth, dispatch, target, or diagnostic behavior changed.
- Validation performed: `node --check docs\js\bots.js`; `node --test tests\bots-polling-state.test.mjs tests\notifications-runtime-authority.test.mjs` (`46 passed`); `git diff --check`.
- Remaining live verification: in Admin Bots, open the Twitch debug drawer and recent-message block, wait through multiple automatic polls, run manual refresh, and confirm latest status/current-session text updates while drawers and scroll remain stable.

- Live root/path verification: `G:\StreamSuites-Dashboard` was unavailable in this shell, and `C:\NEPTUNE LOCAL\GIT\StreamSuites-Dashboard` was the accessible Dashboard checkout. No local Dashboard dev-server process was detected on the usual local ports, so no different live Dashboard process path could be attributed during inspection.
- Admin Bots drawer snapping root cause: the actual polling path is `startPollingAsync()` -> `refresh()` -> `fetchPayload(/api/admin/bots/status)` -> `render(normalized)`, and manual refresh uses `reloadBotsSafely()` -> `refresh()`. Both paths could fully replace `#bots-table-body` with `innerHTML`; state preservation was duplicated around body replacement only, while surrounding counter/platform/manual DOM writes could still shift scroll, and legacy session-suffixed row UI keys were still retained during pruning.
- Admin Bots snapping fix: row UI state is now normalized to stable `creator_id + platform` keys, legacy session-suffixed keys migrate to that stable key, and missing-row pruning only keeps visible creator/platform keys. Background polling and manual refresh now share `renderWithPreservedBotsUiState()` so expanded creator rows, Twitch platform detail cards, debug drawers, recent-message expansion, scroll, and practical focus restoration survive live data updates and `session_id` rotation.
- Twitch 400/4003 debug display fix: the Bots debug panel now surfaces `current_subscription_secondary_blocker_code`, `welcome_to_subscription_elapsed_ms`, `welcome_timeout_seconds`, `websocket_closed_before_subscription`, and current-attempt subscription HTTP/auth/response fallback fields. Admin should show `websocket_connected_then_subscription_failed`, `subscription_session_expired_or_disconnected`, `connection_unused_4003`, and preserved request/response diagnostics after close.
- Human note: open Bots / Runtime Status drawers should stay open while row text updates every poll. A Twitch `session_id` rotation should not collapse creator/platform/debug/recent-message state; if the row disappears, only that row's saved state is pruned.
- Kick protection note: Dashboard changes are limited to `docs/js/bots.js` and the focused Bots polling-state tests. Kick behavior, routing, dispatch, and diagnostics contracts are not changed.
- Validation performed: `node --check docs/js/bots.js`; `node --test tests/bots-polling-state.test.mjs` (`7 passed`); `git diff --check`.
- Remaining live verification: open the Twitch creator row, debug drawer, and recent messages in Admin Bots, wait through background polling and run manual refresh, confirm scroll/open state remains stable while status text updates, then verify the next Twitch HTTP 400/4003 attempt displays the preserved timing and subscription diagnostics.

- Admin Bots Twitch handshake display root cause: Runtime/Auth now reports an active pre-welcome EventSub WebSocket opening-handshake timeout as the current blocker, but the debug grid did not surface the new connection-stage, transport-error, retry-attempt, or next-retry fields in the current-attempt panel.
- Admin Bots Twitch handshake display fix: `docs/js/bots.js` now renders current `connection_stage`, `transport_error_code` / message, retry attempt, and next retry time from `diagnostics.current_attempt` with session-snapshot fallback. Historical failed-ping-pong and old dispatch failures remain in history/timeline only, and polling/drawer preservation remains unchanged.
- Human note: while Twitch reconnects after `timed out during opening handshake`, Admin should keep the row visible and show `websocket_open_timeout`, `eventsub_opening_handshake`, retry attempt, and next retry time as the active current state. After a successful EventSub welcome/subscription, the active handshake blocker should clear and the current panel should return to listening/current-session fields.
- Kick protection note: Dashboard changes are limited to the Bots debug rendering and Bots polling-state tests. Kick behavior, routing, dispatch, and diagnostics contracts are not changed.
- Test maintenance note: the existing static Bots assertion now matches the current `Current Session Pipeline` label instead of the older `Trigger Pipeline` text; no UI label was changed.
- Validation performed: `node --check docs\js\bots.js`; `node --test tests\bots-polling-state.test.mjs tests\notifications-runtime-authority.test.mjs`; `git diff --check`.
- Remaining live verification: open the Twitch creator row/platform/debug drawer, confirm handshake timeout appears in the active current panel while older failed-ping-pong/dispatch entries stay under history, then wait through polling/retry updates and confirm the drawer remains open.

- Admin Bots current-vs-history root cause: the debug panel rendered trigger/dispatch fields from the merged diagnostics timeline/export snapshot, so a prior Twitch session's failed-ping-pong or old dispatch authorization error could appear beside a newer listening EventSub session as if it were current.
- Admin Bots current-vs-history fix: Bots debug now prefers Runtime/Auth `diagnostics.current_attempt.current_session_*` fields for active Twitch session status, keepalive, notification, chat, trigger, and dispatch evidence. Historical timeline entries remain visible and are labeled as history; active current-session status can show `listening_no_messages_yet`, `waiting_for_current_session_chat_message`, and `no_current_session_dispatch_attempt` until a fresh message/dispatch arrives.
- Admin drawer snapping root cause/fix: the live snapping path was the full status polling refresh with session-id rotation; state now remains keyed by stable `creator_id + platform` with session id as secondary, refresh/manual refresh use the same renderer, scroll/focus are restored around table replacement, and tests cover an open Twitch debug drawer surviving polling plus session id rotation. Polling remains enabled.
- Human note: before the first current-session Twitch message, Admin should show listening/keepalive plus no current dispatch attempt, not old failure text. After a fresh `!ping`, it should show current inbound command, sender, trigger match count, dispatch HTTP/message, and keep old failures only in diagnostics/history.
- Kick protection note: Dashboard changes are limited to `docs/js/bots.js` and Bots tests; no Kick source or behavior was changed.
- Validation performed: `node --check docs/js/bots.js`; `node --test tests/bots-polling-state.test.mjs`.
- Remaining live verification: open the Twitch creator row/platform/debug drawer, wait through background polling and a session id rotation, confirm drawers and scroll remain stable, then send a fresh current-session `!ping` and verify active fields update while old failures stay under history.

- Admin Bots polling/drawer reset root cause: the previous preservation path still used rotating `session_id` as part of the primary row UI identity and restored scroll with a timer that could run in the same queue as the poll loop. Full table refreshes could therefore replace open DOM, miss the refreshed row when the session ID rotated, and make existing tests consume a restoration timer instead of the real poll refresh.
- Admin Bots polling/drawer reset fix: row UI state now keys primarily by stable `creator_id + platform`, migrates older session-suffixed keys, preserves expanded creator rows, platform detail drawers, Twitch debug drawers, recent-message expansion, scroll, and focus across polling and manual refresh, and prunes saved state only after the row disappears. The second scroll/focus restore uses `requestAnimationFrame` or a microtask fallback, so polling remains enabled and row content still updates.
- Human note: open Bots / Runtime Status inspection panels should no longer collapse during background polling or manual refresh; if a Twitch worker rotates `session_id`, the drawer follows the creator/platform row unless that row disappears.
- Kick protection note: Dashboard changes are limited to the Bots UI state-preservation path and tests. Kick behavior, routing, dispatch, and diagnostics contracts are not changed.
- Validation performed: `node --check docs\js\bots.js`; `node --check tests\bots-polling-state.test.mjs`; `node --test tests\bots-polling-state.test.mjs tests\notifications-runtime-authority.test.mjs`.
- Remaining live verification: open a Twitch creator row, platform detail, debug drawer, and recent messages on Bots / Runtime Status, wait for background polling and run manual refresh, and confirm the open drawers and scroll position remain stable while row text updates.
...
- Bulk market and inventory edits reuse the existing Runtime/Auth single-item admin PATCH endpoints sequentially; no Dashboard-owned economy state or new Runtime/Auth batch authority was added.
- Human note: Admin Economy / Inventory should now feel like one operational workspace: select an identity compactly, inspect wallet/inventory with most of the width, open audit history only when needed, and bulk-edit safe item fields without losing the individual editor modals.
