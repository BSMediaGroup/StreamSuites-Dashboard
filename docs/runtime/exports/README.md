...
- `version.json` is the legacy Runtime system version/build compatibility projection. Existing dashboards and public pages may continue consuming it during migration; it is not the component event ledger.
- `version-registry.json` is the deterministic public-safe projection of the Runtime/Auth registry. Its current no-event baseline contains 13 policy-authorized components, including the Desktop Admin product family and Alerts Client, while both products retain deferred null semantic baselines. It exposes the system semantic version, current ecosystem revision/build, registered public components, initialized public product baselines, and public-safe latest build/deployment/publication summaries. It excludes reporter workflow policy, OIDC claims, tokens, database paths, full internal metadata, and private component mappings.
- Existing exports (clips, polls, tallies, scoreboards) remain unchanged and continue to serve as reference data snapshots.
- `wheels.json` publishes authoritative public wheel artifacts, including entries, palette/presentation settings, creator linkage, persisted default wheel-vs-scoreboard view, and truthful import metadata notes.

The registry export is regenerated atomically after a newly accepted registry event and during safe registry initialization. Regeneration alone never allocates a build or changes a semantic version. A baseline with no accepted events truthfully uses ecosystem revision `0` and a null latest build ID. `GET /api/public/version-registry` returns the same contract; the authenticated admin API provides a richer bounded view.
