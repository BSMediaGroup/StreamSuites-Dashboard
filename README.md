...
- The Alerts workspace is a compact Runtime-backed control center with an operational hero, dedicated System Status summary, master/detail rule workspace, target status cards, safe test preview, delivery feed, responsive section navigation, and reduced-motion behavior. The existing shell, route, auth, dark visual identity, paging, location/page context, and Runtime/Auth authority remain unchanged.
- Rules support Runtime/Auth-backed StreamSuites, DanielClancy, `system_status_change`, and `system_monitor_state` events, including backend template variables and scope controls. Status rule/history cards add a restrained cyan identity, source and transition context, safe status links, and event-family/severity/destination history filters.
- Ordinary create, edit, enable/disable, delete, and preference actions call granular Runtime/Auth endpoints and refresh the canonical result; they do not replace the ruleset or reset unrelated preferences. Protection affordances consume backend `protected` metadata rather than a Dashboard copy of legacy UUIDs.
- Whole-configuration PUT is reserved for an explicitly staged backup import. The UI reports rule/preference diffs, asks separately about deletions, sends the reviewed configuration revision, and treats a 409 as a non-overwriting stale-document conflict. Alert preferences continue to manage master state, timezone-aware quiet hours, and per-destination enabled/minimum-severity controls.
